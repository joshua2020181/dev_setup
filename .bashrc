...
# python stuff
export PYLINTRC="/home/joshua/kiwi/.pylintrc"
...
sshp() {
	if [ $1 == "192.168.2.100" ]; then
		ssh root@192.168.2.100 -o StrictHostKeyChecking=no
	elif [ $1 == "192.168.2.103" ]; then
		sshpass -p noiuseaspoon. ssh root@192.168.2.103 -o StrictHostKeyChecking=no
	elif [ $1 == "192.168.2.104" ]; then
		sshpass -p noiuseaspoon. ssh root@192.168.2.104 -o StrictHostKeyChecking=no
	else
		ssh root@$1
	fi
}
...
if [ "$color_prompt" = yes ]; then
 PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u:\[\033[01;31m\]$(parse_git_branch)\[\033[01;34m\]\w\[\033[00m\]\$ '
else
